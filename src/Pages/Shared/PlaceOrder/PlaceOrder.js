...
const PlaceOrder = () => {
  ...
  const onSubmit = data => {
    ...
    console.log(data);
      axios.post('https://gentle-mesa-29497.herokuapp.com/order',data)
      .then(res => {
          console.log(res);
    ...
    useEffect(()=>{
        fetch(`https://gentle-mesa-29497.herokuapp.com/placeOrder/${planId}`)
        .then(res => res.json())
        .then(data => {
            setPlan(data);
        })
        ...
        <Container>
            <h2 className="text-center my-2">Place Order</h2>
            {
                ...
                <Col md={6}>
                    ...
                    <p><span className="badge bg-success">Floor: {plan.floor}</span></p>
                    <p><span className="badge bg-success">Rooms: {plan.Rooms}</span></p>
                    <p><span className="badge bg-success">Total Area: {plan.TotalArea}</span></p>
                    <p><span className="badge bg-success">BathRoom: {plan.BathRoom}</span></p>
                    <p><span className="badge bg-success">Windows: {plan.Windows}</span></p>
                    </div>
                    <p>{plan.shortDec}</p>
                    <p>Apartment Rent: {plan.rent}$</p> 
                </Col>
                <Col md={6}>
                {/* Hook Form  */}
